...
| 自定义机组内 PLR / 冷量分配 | 顶层 `load_allocation` 工厂，见 [分配接口](CHILLER_SCHEDULING.md) | 否 |
...
组合内冷量分配也可独立配置：默认按额定冷量加权，自定义函数可以给各机不同 PLR，返回 ID 到冷量 kW 的映射。分配后才调用本指南的主机功率预测器。最小开停机时间、历史累计运行时间及优先规则见 [主机调度与负荷分配指南](CHILLER_SCHEDULING.md)。
